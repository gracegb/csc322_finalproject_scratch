...
class ProviderCalendarScreen extends ChangeNotifier {
  ...
  for (var eventList in _events.values) {
    for (var event in eventList) {
      if (event.repeat != 'None' && _isRecurringEventOnDate(event, normalizedDate)) {
  ...
  final normalizedDate = DateTime(date.year, date.month, date.day);
  DateTime occurrenceDate = DateTime(event.startTime.year, event.startTime.month, event.startTime.day);
